...
app.whenReady().then(async () => {
  ...
  const videoTransport = JSON.parse(await win.webContents.executeJavaScript(`(async()=>{
    ...
    let mixerRow=document.querySelector('[data-audio-layer-id="'+selectedLayer().id+'"]');
    let mixerFader=mixerRow.querySelector('.audio-fader');mixerFader.value='31';mixerFader.dispatchEvent(new Event('input',{bubbles:true}));mixerFader.dispatchEvent(new Event('change',{bubbles:true}));await wait(80);
    const mixerLevel={preview:selectedLayer().volume,program:findProgramLayer(selectedLayer())?.volume,previewGain:meterRecordForLayer(selectedLayer())?.monitorGain?.gain?.value,programGain:programAudioRecordForLayer(findProgramLayer(selectedLayer()))?.programGain?.gain?.value,label:mixerRow.querySelector('.audio-fader-value').textContent};
    mixerRow.querySelector('.audio-mute').click();await wait(80);
    const mixerMuted={preview:selectedLayer().muted,program:findProgramLayer(selectedLayer())?.muted,previewGain:meterRecordForLayer(selectedLayer())?.monitorGain?.gain?.value,programGain:programAudioRecordForLayer(findProgramLayer(selectedLayer()))?.programGain?.gain?.value};
    mixerRow=document.querySelector('[data-audio-layer-id="'+selectedLayer().id+'"]');mixerRow.querySelector('.audio-mute').click();await wait(50);
    mixerRow=document.querySelector('[data-audio-layer-id="'+selectedLayer().id+'"]');mixerFader=mixerRow.querySelector('.audio-fader');mixerFader.value='80';mixerFader.dispatchEvent(new Event('input',{bubbles:true}));mixerFader.dispatchEvent(new Event('change',{bubbles:true}));await wait(50);
    ...
    return JSON.stringify({visible,previewOnly,programOnly,bothAudio,previewPlaying,taken,mixerLevel,mixerMuted,independent,paired,stopped,target:videoTransportTarget,clock:document.getElementById('inspVideoClock').textContent});
  })()`));
  ...
  check('COMPOSITOR_VIDEO_MIXER_CONTROLS_LIVE_GAIN_OK', Math.abs(videoTransport.mixerLevel.preview - .31) < .001 && Math.abs(videoTransport.mixerLevel.program - .31) < .001 && Math.abs(videoTransport.mixerLevel.previewGain - .31) < .001 && Math.abs(videoTransport.mixerLevel.programGain - .31) < .001 && videoTransport.mixerLevel.label === '-10.2 dB' && videoTransport.mixerMuted.preview === true && videoTransport.mixerMuted.program === true && videoTransport.mixerMuted.previewGain === 0 && videoTransport.mixerMuted.programGain === 0, JSON.stringify({mixerLevel:videoTransport.mixerLevel,mixerMuted:videoTransport.mixerMuted}));
  ...
  const authored = JSON.parse(await win.webContents.executeJavaScript(`(async()=>{
    ...
    const audioTaken=takeSelectedLayer();
    let audioRow=document.querySelector('[data-audio-layer-id="'+audioLayer.id+'"]');
    const fader=audioRow.querySelector('.audio-fader');fader.value='62';fader.dispatchEvent(new Event('input',{bubbles:true}));const volumeAfterInput=currentScene().layers.find(layer=>layer.id===audioLayer.id)?.volume;const programVolumeAfterInput=findProgramLayer(audioLayer)?.volume;fader.dispatchEvent(new Event('change',{bubbles:true}));const volumeAfterChange=currentScene().layers.find(layer=>layer.id===audioLayer.id)?.volume;
    audioRow=document.querySelector('[data-audio-layer-id="'+audioLayer.id+'"]');audioRow.querySelector('.audio-mixer-advanced').open=true;
    const monitor=audioRow.querySelector('.audio-monitoring');monitor.value='monitor-only';monitor.dispatchEvent(new Event('change',{bubbles:true}));const monitoringAfterChange=currentScene().layers.find(layer=>layer.id===audioLayer.id)?.audioMonitoring;const programMonitoringAfterChange=findProgramLayer(audioLayer)?.audioMonitoring;
    audioRow=document.querySelector('[data-audio-layer-id="'+audioLayer.id+'"]');audioRow.querySelector('.audio-mute').click();const mutedAfterClick={preview:currentScene().layers.find(layer=>layer.id===audioLayer.id)?.audioEnabled,program:findProgramLayer(audioLayer)?.audioEnabled};
    audioRow=document.querySelector('[data-audio-layer-id="'+audioLayer.id+'"]');audioRow.querySelector('.audio-mute').click();
    ...
    const audioState={layer:cloneState(currentScene().layers.find(layer=>layer.id===audioLayer.id)),programLayer:cloneState(findProgramLayer(audioLayer)),input:cloneState(liveInputDefinition(audioLayer.inputId)),audioTaken,volumeAfterInput,programVolumeAfterInput,volumeAfterChange,monitoringAfterChange,programMonitoringAfterChange,mutedAfterClick,rows:document.querySelectorAll('#audioMixerRows .audio-channel').length,meters:document.querySelectorAll('#audioMixerRows .audio-meter').length,programRoute:activeProgramAudioRouteValue(),programDevice:S.programAudioDeviceId,programStateDevice:programState.programAudioDeviceId};
  ...
  check('COMPOSITOR_LIVE_INPUT_MIXER_CONTROLS_PROGRAM_OK', authored.audioState.audioTaken && authored.audioState.programLayer && authored.audioState.volumeAfterInput === 0.62 && authored.audioState.programVolumeAfterInput === 0.62 && authored.audioState.monitoringAfterChange === 'monitor-only' && authored.audioState.programMonitoringAfterChange === 'monitor-only' && authored.audioState.mutedAfterClick.preview === false && authored.audioState.mutedAfterClick.program === false && authored.audioState.programLayer.audioEnabled === true, JSON.stringify(authored.audioState));
